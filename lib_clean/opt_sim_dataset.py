...
class TensorFile:
    ...
    def path(self):
        return (
            self.data_root.joinpath(f"block{self.block_idx}")
...
class ConsecutiveOutputsDataset(Dataset):
    ...
    def _compute_sample_len(self, block_id: int, sample_id: int):
        return len(
            list(
                self.data_root.joinpath(f"block{block_id}")
    ...
    def __getitem__(self, idx: int):
        ...
        sample_idx, sample_offset = self._find_in_prefix(
            prefix=self.sample_prefix[start_layer_idx], value=idx - block_offset
        )
        token_idx = idx - block_offset - sample_offset

        x = TensorFile(
            start_layer_idx, sample_idx, token_idx, self.data_root, self.num_layers
        )
        v1 = x.advance(1)
        v2 = x.advance(self.n + 1)
        blocks = torch.tensor(
            [start_layer_idx, start_layer_idx + 1, start_layer_idx + self.n + 1],
            dtype=torch.long,
        )
