...
def evaluate_checkpoint(model_path: str, tasks: List[str], assistance_config: Optional[AssistanceConfig]=None):
    ...
    all_results = dict()
    for task in tasks:
        task_name, task_fewshot = task.split('@')
        all_results[task_name] = evaluator.simple_evaluate(
            model=lm_obj,
            tasks=[task_name],
...
def eval_model(model_name: str, tasks: List[str], assistance_config: Optional[AssistanceConfig]=None):
    all_results = {task.split('@')[0]: [] for task in tasks}
    ...
    for task in tasks:
        ...
        all_results[task_name].append(format_result(results, task_name))
