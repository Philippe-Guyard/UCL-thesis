...
class MetricTracker:
    ...
    def update(self, y_pred, y_true):
        ...
        self.predicted_indices += y_pred.argmax(-1).tolist()
    ...
    def get_metrics(self):
        ...
        reward = torch.where(predicted_tensor > best_tensor, -predicted_tensor, predicted_tensor)
        average_reward = reward.float().mean().item()
...
def reinforce_loss(y_pred, y_true):
    '''
    y_pred: logits (seqlen, n_blocks)
    y_true: correct index (seqlen,)
    ''' 
    seqlen = y_pred.size(0)
    y_prob = torch.softmax(y_pred, dim=-1)
    selected_action = torch.multinomial(y_prob, num_samples=1)
    reward = torch.where(selected_action > y_true, -selected_action, selected_action) 
    log_prob = torch.log(y_prob[torch.arange(seqlen), selected_action])
