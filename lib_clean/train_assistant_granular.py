...
if not objective_config.use_distil_target:
    collect_modules.update({'block', 'mlp', 'attn'})
...
def _generate_synthetic_data_angles(batch, device):
    ...
    bsz, seq_len = tokens.input_ids.shape
    mlp_ins = torch.stack([
        TensorStorage._cur_sample[f'block{block_idx}_mlp_in_first'][0].reshape(bsz, seq_len, -1)
        for block_idx in range(0, n_blocks)
    ]) 
    mlp_outs = torch.stack([
    ...
    attn_similarities = F.cosine_similarity(
        attn_ins,
        attn_outs,
        dim=-1,
    )
    # (2 * n_blocks, bsz, seq_len)
    cos_similarities = torch.cat((mlp_similarities, attn_similarities), dim=0)
    # (bsz, 2 * n_blocks, seq_len), (bsz, seq_len, 2 * n_blocks) 
    cos_similarities = cos_similarities.transpose(0, 1).transpose(1, 2)
    # (bsz, 2 * n_blocks, seq_len)
...
def get_criterion():
    if objective_config.objective == 'regression':
        ...
        # Now we have n_modules = 2 * n_blocks
        return topk_mse(n_blocks)
    else:
...
print(cfg)
model = GPT2ForLayerPruning(cfg, teacher_model.config.hidden_size, 2 * n_blocks).cuda()
...
def compute_val_metrics(model, test_loader, criterion):
    model.eval()
    metric_tracker = MetricTracker(objective_config.objective, criterion, 2 * n_blocks) 
